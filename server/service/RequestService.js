...
class RequestService {
    ...
    async addRequest(request) {
        await this.dbConn.addRecord(this.type, request).then((result) => {  
            request = result;
        })
        .catch(function (err) {
            console.warn(err);
    ...
    async closeRequest(id, closeReason) {
        let request;
         await this.getRequest(id).then(async (result) => { 
            ...
            const condition = { '_id' : ObjectId(id) }, newValues = {closeReason, isClosed: true}
            await this.dbConn.updateRecord(this.type, condition, newValues).then((result) => {  
